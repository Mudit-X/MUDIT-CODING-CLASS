...
def greet_user():
    print(Fore.CYAN + "Hello I'm TravelBot, your virtual travel following:")
...
def process_input(user_input):
    #Convert input to lowercase, remove leading/trailling spaces, and replace multiple spaces with one
...
def provide_recommendations():
    print(Fore.CYAN + "TravelBot: Sure! Are you intrested in beaches, mountains, or cities?")
    preference = input(Fore.YELLOW + "You: ")
    preference = process_input(preference)#Normalize the input

    if preference in destinations:
        suggestion = random.choice(destinations[preference])
        print(Fore.GREEN + f"TravelBot: How about visitng {suggestion}?")
...
def offer_packing_tips():
    ...
    print(f"{Fore.CYAN}TravelBot: How many days will you be staying? (Please only enter numbers)")
