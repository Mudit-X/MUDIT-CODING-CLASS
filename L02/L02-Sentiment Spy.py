...
def show_processing_animation():
    print (f"{Fore.CYAN} Detecting sentiment clues", end="") #Starting animation text
    for _ in range(3): #Loop to print three dots for the animation
        ...
        sys.stdout.flush() # Force the terminal to display the output
...
def analyze_sentiment(text):
    ...
    try:
        ...
        #Catergory the sentiment based on threshols
        if sentiment > 0.75:
            ...
            return f"\n{Fore.GREEN} Very Negative sentiment detected, Agent {user_name}! (Score: {sentiment:.2f})" 
...
def execute_command(command):
      ...
      if command == "summary":
        ...
        positive_count = negative_count = negative_count = 0 #Rest couters
        ...
        return (f"{Fore.CYAN} Available commands:\n"
                ...
                f"- Type 'exit' to conclude you mission and leave the chat.\n")
      else:
...
def get_valid_name():
     """
     Continuously prompts the user until they provide a valid name containg only alphabetic characters.
     """
     while True:
